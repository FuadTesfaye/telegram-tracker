    ...
    <div className="space-y-3.5">
      ...
      {accounts.length === 0 ? (
        ...
        <>
          ...
          <div className="bg-[#10131d] border border-white/[0.08] p-1 rounded-2xl flex items-center justify-between text-xs font-semibold">
            <button
              onClick={() => {
                setRoastLevel("friendly");
                hapticFeedback("light");
              }}
              className={`flex-1 py-1.5 rounded-xl transition-all flex items-center justify-center gap-1 tap-effect ${
                roastLevel === "friendly"
          ...
          {roastData && (
            <div className="bg-[#12151e] border border-white/[0.08] rounded-2xl p-4 space-y-3.5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono font-bold tracking-wider uppercase text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-500/20 flex items-center gap-1.5">
                ...
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${isRoasting ? "animate-spin text-blue-400" : ""}`} />
                </button>
              </div>
              ...
              <div>
                ...
                <div className="text-xs text-blue-400 font-medium mt-0.5 flex items-center gap-2">
              ...
              <div className="p-3.5 bg-[#181c28] border border-white/[0.06] rounded-xl relative">
                <Quote className="w-4 h-4 text-zinc-600 mb-1 opacity-60" />
                <p className="text-xs text-zinc-200 leading-relaxed font-medium">
                  {roastData.roastText || roastData.roast}
                </p>
              </div>

              <div className="p-2.5 bg-white/[0.03] border border-white/[0.05] rounded-xl text-xs font-medium text-zinc-300 text-center">
                {roastData.verdict}
              </div>

              <button
                onClick={copyRoast}
                className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 border border-blue-400/20 text-white text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2 tap-effect shadow-sm"
              >
          ...
          <div className="bg-[#12151e] border border-white/[0.08] rounded-2xl p-3.5 space-y-2.5">
            ...
            <div className="grid grid-cols-5 gap-1.5 text-center">
              {[
                ...
                <div
                  key={tier.name}
                  className="p-2 rounded-xl bg-[#181c28] border border-white/[0.04] flex flex-col items-center"
                >
                  <span className="text-base">{tier.icon}</span>
                  <span className="text-[10px] font-bold text-zinc-200 mt-1">
                    {tier.name}
                  </span>
              ...
              {achievements.map((item) => (
                <div
                  ...
                  className={`p-3 rounded-xl border transition-all flex items-start gap-2.5 ${
                    item.unlocked
                      ? "bg-[#181c28] border-white/[0.08]"
                      : "bg-[#181c28]/40 border-white/[0.03] opacity-40"
                  }`}
                >
